...
void QY_run()	//按下操作 正在照射		
{
	//QRJ_ZX__QY_RUN();
  QRJ_ZX__QY_RUN(ZX_mm_to_s);	

	//触发 照射完成
	if(mJ_doing*1000 >= quyu_j  ||  TIME_static<=0  ||  baifenbi_T >=100)
	{		
...
{u8 L_T_S_FLAG;
	...
	switch(L_T_S_FLAG)
	{
		case 1:
		{
			TIME_static=quyu_j/change_P_uW_doing;
			break;
		}
		...
		switch(L_T_S_FLAG)
	{
		case 1:
		{
			...
			TIME_static=((float)(quyu_j - mJ_doing*1000))	/ (change_P_uW_doing) ;
			break;
		}
...
void 	QY_prepare()	//在当前页面 但未操作
{
  ...
  u16 ResTime_min;	
	...
	ReadDGUS(0x5101,(uint8_t *)&QY_mj,sizeof(QY_mj));  	//读取杀灭log值 范围1~5
	switch(QY_mj)
	{
		case 1:
		{
			quyu_j	=	1000;			//单位是uj 而不是mj
			break;
		}
				case 2:
		{
