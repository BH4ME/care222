...
void RTC_SET_IN()
{
	u8 RTC_H[4];	//2023 02 18  年月日
								//0	 1	2	 3
	u8 RTC_L[4];	//00	12 45 23	时分秒
								//0		1	 2	3
	
 u8 RTC_6[6]={0};
	static u8 RTC_9C[2]={0x5a,0xa5};
	 u32 RTC_H_HEX;
	static u32 RTC_L_HEX;
	//long i;
// u64 RTC_8;
	ReadDGUS(0X1300,(u8 *)&RTC_H,sizeof(RTC_H));
	ReadDGUS(0X1305,(u8 *)&RTC_L,sizeof(RTC_L));
...
//	else RTC[3]=1;
	...
	if(RTC_6[1]>0&&RTC_6[1]<=12)	//月
	{
		if(RTC_6[2]<=31&&RTC_6[2]>0)	//日
		{
			...
			WriteDGUS(0x9d, (u8 *)&RTC_6,sizeof(RTC_6));	
			WriteDGUS(0x9c, (u8 *)&RTC_9C,sizeof(RTC_9C));	
...
{					  
					...
					//	u16  Va=ReadDGUS(0x009C); //0x5AA5
						u16  Va;
						u8  zero[2]={0,0};
						ReadDGUS(0x009C,(u8 *)&Va,2);
						if(Va==0x5AA5)
						{			
									u8 N=0,S=0;
									bit  Flog=1;
